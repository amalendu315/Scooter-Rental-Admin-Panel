...
import { mockRiders, mockRentals, mockPayments } from '@/lib/data';
            ...
            <Tabs defaultValue="profile" className="w-full">
                ...
                <TabsContent value="profile" className="mt-4">
                    <Card>
                        ...
                        <CardContent><RiderForm rider={rider} /></CardContent>
                    </Card>
                </TabsContent>
                <TabsContent value="rentals" className="mt-4">
                    <Card>
                        ...
                        <CardContent>
                            <Table>
                                <TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Vehicle</TableHead><TableHead>Period</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {riderRentals.map(r => (
                                        <TableRow key={r.id}>
                                            ...
                                            <TableCell><Badge variant={r.status === 'completed' ? 'default' : r.status === 'ongoing' ? 'secondary' : 'destructive'} className={cn(r.status === 'ongoing' && 'bg-blue-500 text-white')}>{r.status}</Badge></TableCell>
                ...
                <TabsContent value="payments" className="mt-4">
                    <Card>
                        ...
                        <CardContent>
                            <Table>
                                <TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Date</TableHead><TableHead>Method</TableHead><TableHead className="text-right">Amount</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {riderPayments.map(p => (
                                        <TableRow key={p.id}>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {rider.kycDocuments && rider.kycDocuments.length > 0 ? (
                                ...
                                <p className="text-center text-muted-foreground py-4">No KYC documents uploaded.</p>
                            )}
                        </CardContent>
                    </Card>
                </TabsContent>
