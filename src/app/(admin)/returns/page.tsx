...
export default function ReturnsPage() {
    ...
    useEffect(() => {
        setLoading(true);
        mockApi.listRentals({ pageSize: 100, filters: { status_ne: 'completed' } })
            .then(data => {
                const openRentals = data.rows.filter(r => r.status === 'ongoing' || r.status === 'overdue');

                mockApi.listRentals({ pageSize: 50, sortBy: 'actualReturnDate', filters: { status: 'completed' } })
                    .then(completedData => {
                        const recentReturns = completedData.rows.filter(r => r.actualReturnDate && isAfter(new Date(r.actualReturnDate), subDays(new Date(), 7)));
